...
def main():
    ...
    repo_slug = util.get_origin_repo_slug()
    repo_branch = util.get_origin_branch()
    new_project_tag = "0"
    ...
    print(f"Release repository: {repo_slug} ({repo_branch})")
    build.do_build(last_frida_tag, new_project_tag, repo_slug, repo_branch)
