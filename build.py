...
def create_module_prop(path: Path, project_tag: str, repo_slug: str):
    module_prop = f"""id=frida-web
name=FridaWeb
version={project_tag}
...
def create_module(project_tag: str, repo_slug: str):
    ...
    create_module_prop(PATH_BUILD_TMP, project_tag, repo_slug)
...
def create_updater_json(project_tag: str, repo_slug: str, repo_branch: str):
    logger.info("Creating updater.json")

    updater = {
        "version": project_tag,
        "versionCode": generate_version_code(project_tag),
        "zipUrl": f"https://github.com/{repo_slug}/releases/download/{project_tag}/FridaWeb-{project_tag}.zip",
        "changelog": f"https://raw.githubusercontent.com/{repo_slug}/{repo_branch}/CHANGELOG.md",
    }
...
def package_module(project_tag: str):
    ...
    module_zip = PATH_BUILD.joinpath(f"FridaWeb-{project_tag}.zip")
...
def do_build(frida_tag: str, project_tag: str, repo_slug: str, repo_branch: str):
    PATH_DOWNLOADS.mkdir(parents=True, exist_ok=True)
    PATH_BUILD.mkdir(parents=True, exist_ok=True)

    create_module(project_tag, repo_slug)
    ...
    create_updater_json(project_tag, repo_slug, repo_branch)
